import {
  ...
  function libraryPackagePlugin() {
    return createComposedProjectPlugin('LithiumPackage', [
  ...
  function libaryWorkspacePlugin() {
    return createComposedWorkspacePlugin('LithiumWorkspace', [
      jest(),
      eslint(),
  ...
  function jestAdjustmentsPlugin() {
    return createProjectPlugin('LithiumJest', ({tasks: {test}}) => {
      test.hook(({hooks}) => {
  ...
  function preAndPostBuildPlugin() {
    return createWorkspacePlugin('LithiumExtraBuild', ({api, tasks: {build}}) => {
      build.hook(({hooks}) => {
        hooks.post.hook((steps) => [
          ...steps,
          api.createStep(
            {id: 'LithiumBuild.Post', label: 'Lithium post-build'},
            async (step) => {
              try {
                await step.exec(
  ...
  function rollupAdjustPluginsPlugin() {
    return rollupPlugins((target) => {
      ...
      return [
        replace({
          '{{LITHIUM_VERSION}}': packageJSON.version,
          delimiters: ['', ''],
        }),
        image(),
        styles(stylesConfig),
  ...
  function rollupAdjustOutputPlugin() {
    return createProjectPlugin('LithiumJest', ({tasks: {build}}) => {
      build.hook(({hooks}) => {
